...
const Service = () => {
  ...
  return (
    <section
      className={`w-full h-full border-2 border-red-600 select-none overflow-hidden transition-colors duration-500 ${
  ...
  return (
    ...
    >
      ...
      <img
        src={image}
        alt={title}
        className="w-60 h-60 shrink-0 rounded-3xl object-cover border-2 border-amber-200 relative z-10"
      />
      <div
...
const WaterButton = ({ text, href }) => {
  ...
  return (
    <a href={href} className="relative inline-block">
      <button
        className="relative hover:scale-105 duration-150 overflow-hidden border-2 cursor-pointer border-yellow-100 text-yellow-100 font-bold  p-5 rounded-lg shadow-md group"
        onMouseMove={handleMouseMove}
        onMouseEnter={() => setHovered(true)}
        onMouseLeave={() => setHovered(false)}
      >
