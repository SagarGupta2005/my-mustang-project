...
const Navbar = () => {
  ...
  return (
    <motion.header
      className={`fixed top-0 select-none flex items-center justify-center w-full z-999 transition-all xl:px-16 duration-300 ${
        scrolled
          ? darkMode
            ? " text-white"
...
transition={{
  ...
  type: "tween",
    ...
    >
        ...
        <div className="flex items-center gap-4">
          ...
          {searchOpen && (
            ...
            >
              <input
                type="text"
                className="px-2 py-1 w-40 text-black focus:outline-none"
                placeholder="Search..."
                value={searchQuery}
      ...
      {/* TABLET */}
      <div className="hidden md:flex lg:hidden justify-between items-center px-8 py-2">
        <Link to="/">
          <img
            src="https://cutstyle.true-emotions.studio/wp-content/themes/cutstyle/images/logo.svg"
            alt="Logo"
      ...
      {/* DESKTOP */}
      <div className="hidden xl:flex justify-center items-center px-10 py-5">
        <nav className="flex items-center text-xl font-serif tracking-wide gap-10">
          {menuItems.slice(0, middleIndex).map((item) => (
            <Link
              to={getPath(item)}
              key={item}
      ...
      <AnimatePresence>
        {mobileOpen && (
          <motion.div
            ...
            className={`fixed top-0 left-0 h-full w-80 sm:w-3/4 ${
          ...
          >
            ...
            <div className="flex items-center gap-6 mt-auto pb-10">
              ...
              {searchOpen && (
                <form
                  onSubmit={handleSearchSubmit}
                  className="flex items-center bg-gray-700 rounded overflow-hidden grow"
                >
                  <input
                    type="text"
                    className="px-2 py-1 w-full text-black focus:outline-none"
                    placeholder="Search..."
                    value={searchQuery}
                    onChange={handleSearchChange}
                    autoFocus
